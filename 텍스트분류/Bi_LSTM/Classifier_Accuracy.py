...
file = open("Data\\test_data_shuffled.csv", 'r', encoding='euc-kr')
line = csv.reader(file)
correct = 0
except_handler = 0
for i,info in enumerate(line) :
    try:
        print(i,'번째 테스트 중 입니다.')
        print('정답 = ' ,info[0])
        print('시도 = ',Grade(info[1]))
        if info[0] == Grade(info[1]):
            correct += 1
            print('정답으로 맞춤')
    except:
        except_handler+=1
print("except_handler =",except_handler)
print('9996개 테스트 결과 정확도  = ',(round(correct/(9996-except_handler),2))*100,'%')
